...
WP3_DIR = ROOT_DIR / "data" / "2.harvest_wp3_sensor_observation_data"
...
def count_csv_occurrences(csv_file: Path):
	"""Count rows and expanded occurrences in a WP3 CSV file."""
...
def main():
	if not WP3_DIR.exists():
		raise FileNotFoundError(f"WP3 data directory not found: {WP3_DIR}")

	csv_files = sorted(WP3_DIR.glob("*.csv"))
	if not csv_files:
		print(f"No CSV files found in {WP3_DIR}")
		return
